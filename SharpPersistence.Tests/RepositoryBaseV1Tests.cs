...
public interface ITestRepository : IRepositoryBaseV1<TestEntity>
{
}

public class TestRepository : RepositoryBaseV1<TestEntity, TestDbContext>, ITestRepository
{
...
public class RepositoryBaseV1Tests : IAsyncLifetime
{
    ...
    {
        var value = await _repository.GetOneSubsetAsync(e => e.Value, e => e.Name == "A",
    ...
    {
        var result = await _repository.GetAllSortedAndPaginatedSubsetAsync<string, int>(1, 2, e => e.Name,
            (e => e.Value, false), e => e.Value > 1, true, TestContext.Current.CancellationToken);
        result.Count.ShouldBe(2);
    ...
    {
        var result =
            await _repository.GetAllSortedAndPaginatedAsync(1, 2, (e => e.Value, false), e => e.Value > 1, true,
                TestContext.Current.CancellationToken);
        result.Count.ShouldBe(2);
    ...
    {
        var result =
            await _repository.GetAllSortedSubsetAsync<string, int>(e => e.Name, (e => e.Value, false), e => e.Value > 2,
    ...
    {
        var result = await _repository.GetAllSortedAsync((e => e.Value, false), e => e.Value > 2, true,
            TestContext.Current.CancellationToken);
        result.Count.ShouldBe(2);
    ...
    {
        var result = await _repository.GetAllSubsetAsync(e => e.Name, e => e.Value > 2, true,
            TestContext.Current.CancellationToken);
        result.Count.ShouldBe(2);
        result.ShouldContain("C");
    }
    ...
    {
        var value = await _repository.GetOneSubsetAsync(e => e.Value, e => e.Name == "A", true,
            TestContext.Current.CancellationToken);
        value.ShouldBe(1);
    }
    ...
    {
        var r = await _repository.GetAllSortedAndPaginatedSubsetAsync<string, int>(1, 2, e => e.Name,
    ...
    {
        var r = await _repository.GetAllSortedAndPaginatedSubsetAsync<string, int>(1, 2, e => e.Name,
            (e => e.Value, false), e => e.Value > 1, TestContext.Current.CancellationToken);
        r.All(x => x != "A").ShouldBeTrue();
    }
    ...
    {
        var r = await _repository.GetAllSortedAndPaginatedAsync(1, 2, (e => e.Value, false),
            TestContext.Current.CancellationToken);
        r.Count.ShouldBe(2);
    }
    ...
    {
        var r = await _repository.GetAllSortedAndPaginatedAsync(1, 2, (e => e.Value, false), e => e.Value > 1,
            TestContext.Current.CancellationToken);
        r.All(x => x.Value > 1).ShouldBeTrue();
    }

    [Fact]
    public async Task GetAllSortedAndPaginatedAsync_WithEnableTracking()
    {
        var r = await _repository.GetAllSortedAndPaginatedAsync(1, 2, (e => e.Value, false), true,
            TestContext.Current.CancellationToken);
        r.Count.ShouldBe(2);
    }
    ...
    {
        var r = await _repository.GetAllSortedAndPaginatedAsync(1, 2, (e => e.Value, false), e => e.Value > 1, true,
            TestContext.Current.CancellationToken);
        r.All(x => x.Value > 1).ShouldBeTrue();
    }
    ...
    {
        var r = await _repository.GetAllSortedSubsetAsync<string, int>(e => e.Name, (e => e.Value, false),
            TestContext.Current.CancellationToken);
        r.Count.ShouldBe(4);
    }
    ...
    {
        var r = await _repository.GetAllSortedSubsetAsync<string, int>(e => e.Name, (e => e.Value, false),
            e => e.Value > 1, TestContext.Current.CancellationToken);
        r.All(x => x != "A").ShouldBeTrue();
    }
    ...
    {
        var r = await _repository.GetAllSortedSubsetAsync<string, int>(e => e.Name, (e => e.Value, false), true,
            TestContext.Current.CancellationToken);
        r.Count.ShouldBe(4);
    }
    ...
    {
        var r = await _repository.GetAllSortedSubsetAsync<string, int>(e => e.Name, (e => e.Value, false),
            e => e.Value > 1, true, TestContext.Current.CancellationToken);
        r.All(x => x != "A").ShouldBeTrue();
    }
    ...
    {
        var r = await _repository.GetAllSortedAsync((e => e.Value, false), e => e.Value > 1, true,
            TestContext.Current.CancellationToken);
        r.All(x => x.Value > 1).ShouldBeTrue();
    }
    ...
    {
        var r = await _repository.GetAllSortedAsync((e => e.Value, false), e => e.Value > 1,
            TestContext.Current.CancellationToken);
        r.All(x => x.Value > 1).ShouldBeTrue();
    }
    ...
    {
        var r = await _repository.GetAllSortedAsync((e => e.Value, false), true, TestContext.Current.CancellationToken);
        r.Count.ShouldBe(4);
    }
    ...
    {
        var r = await _repository.GetAllSortedAsync((e => e.Value, false), TestContext.Current.CancellationToken);
        r.Count.ShouldBe(4);
    }
    ...
    {
        var r = await _repository.GetAllSubsetAsync(e => e.Name, e => e.Value > 1, true,
            TestContext.Current.CancellationToken);
        r.All(x => x != "A").ShouldBeTrue();
    }
    ...
    {
        var r = await _repository.GetAllSubsetAsync(e => e.Name, e => e.Value > 1,
            TestContext.Current.CancellationToken);
        r.All(x => x != "A").ShouldBeTrue();
    }
    ...
    {
        var r = await _repository.GetAllSubsetAsync(e => e.Name, true, TestContext.Current.CancellationToken);
        r.Count.ShouldBe(4);
    }
    ...
    {
        var r = await _repository.GetAllSubsetAsync(e => e.Name, TestContext.Current.CancellationToken);
        r.Count.ShouldBe(4);
    }
    ...
    {
        var r = await _repository.GetAllAsync(e => e.Value > 1, true, TestContext.Current.CancellationToken);
        r.All(x => x.Value > 1).ShouldBeTrue();
    }
    ...
    {
        var r = await _repository.GetAllAsync(e => e.Value > 1, TestContext.Current.CancellationToken);
        r.All(x => x.Value > 1).ShouldBeTrue();
    }
    ...
    {
        var r = await _repository.GetAllAsync(true, TestContext.Current.CancellationToken);
        r.Count.ShouldBe(4);
    }
    ...
    {
        var r = await _repository.GetAllAsync(TestContext.Current.CancellationToken);
        r.Count.ShouldBe(4);
    }
